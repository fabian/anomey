...
function smarty_function_media($params, &$smarty)
{
    if (empty($params['file'])) {
        $smarty->_trigger_fatal_error("[media] parameter 'file' cannot be empty");
        return;
    }
    
    $path = $smarty->get_template_vars('processor')->getURL()->getPath();
    $path .= $smarty->get_template_vars('model')->getMediaPath() . '/';
